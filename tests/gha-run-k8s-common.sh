...
function deploy_rke2() {
	...
	#   and the CNI (flannel), which are required for pods to schedule, network,
	#   and resolve names.
	# - Use flannel instead of the default canal (calico+flannel) CNI, matching
	#   the lighter k3s setup that already passes these tests.
